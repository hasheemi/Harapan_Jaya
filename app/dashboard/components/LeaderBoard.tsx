...
export class LeaderBoard extends PureComponent {
  render() {
    return (
      <div className="w-full card card-xs bg-base-200 p-4">
        <span className="text-xl font-bold retext pb-2 pl-2">LeaderBoard</span>
        {LeaderBoardItem.map((item, index) => (
          <div key={index}>
            <button className="w-full flex flex-row items-center justify-center hover:bg-base-300 transition duration-150 rounded-full">
            ...
            {index != LeaderBoardItem.length - 1 && (
              <div className="h-[0.5] rounded my-2 bg-base-300"></div>
            )}
          </div>
        ))}
      </div>
